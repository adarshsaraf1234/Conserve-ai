...
async def retrieve(
    ...
    with make_retriever(config) as retriever:
        print("state.queries[-1]", state.queries[-1], "config", config)
        response = await retriever.ainvoke(state.queries[-1], config)
        print("retrieved_____docs", response)
...
async def route(
    ...
    print("mcp_server response", response)
    mcp_server = (
...
async def refine_tool_call(
    ...
    print("state.current_tool", state.current_tool)
