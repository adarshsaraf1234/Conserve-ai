...
def load_chat_model(fully_specified_name: str) -> BaseChatModel:
    ...
    if provider == "openai" and OPENAI_API_ENDPOINT is not None:
        OPENAI_API_KEY = os.getenv('OPENAI_API_KEY')
        if OPENAI_API_KEY is None:
            raise ValueError("OPEN_AI_API_KEY env variable must be provided if OPEN_AI_API_ENDPOINT is given")
        
        return init_chat_model(
            model,
            model_provider=provider,
            base_url= OPENAI_API_ENDPOINT,
            configurable_fields=["response_format"],  # Make it configurable
            model_kwargs={
                "temperature": 0.7,
                # response_format will only be used if explicitly configured
            }
        )
    else:
        return init_chat_model(model, model_provider=provider, streaming=False)     
